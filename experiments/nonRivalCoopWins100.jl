...
coopPropRange = [0.02 * i for i in 1:48]
nTrials = 200 
@time resultDf, seriesDf = segDistAvg(mySimulator, numTrials = nTrials, coopPropRange = coopPropRange, numSamples = numSamples)


CSV.write("$root/data/nonRivalCoopWins100.csv", resultDf)
CSV.write("$root/data/nonRivalCoopWins100TS.csv", seriesDf)
